...
본 문서는 현재 구축된 쿠팡 물류창고 관제 시스템(Control Tower)의 한계점과 문제점을 분석하고, 이를 극복하기 위한 데이터베이스 구조 정규화, QR코드 시스템 도입, 이중 버퍼(Double Buffer) 물리 레이아웃 설계, Redis 작업 우선순위 지정, 관제 단일장애점(SPOF) 대응, 그리고 외부 엑셀/CSV 데이터 연동을 통한 동적 택배 명단 로드 방안을 구체적으로 정리한 계획서입니다.
...
> - 1Hz 주기의 `dispatch_workstations_keepalive()` 백그라운드 스케줄러를 통한 자동 빈 작업대 보충 및 B구역 대기 작업대 승격(Promotion) 로직 적용.
> - 3번째 슬롯 적재 완료 시점에 다음 예비 작업대를 B구역으로 자동 호출하는 3-슬롯 Look-ahead 메커니즘 연동 완료.

AMR 이송 속도가 로봇의 적재 속도보다 느려 발생하는 병목 및 적재 로봇 대기 현상을 해결하기 위해 작업대 임시 적재 구역을 이중화하고 동적으로 관리합니다.
...
* **동작 루프**:
  ...
  4. 8번째 슬롯까지 적재가 완료(완충)되면, 관제탑은 완충 작업대를 포장존 또는 창고로 회수(`RETRIEVE_FULL_WORKSTATION`)하고, 동시에 B 구역에 대기 중이던 예비 작업대를 A 구역으로 승격(`DEPLOY_EMPTY_WORKSTATION`)시킵니다.
...
## 🛡️ 5. 관제 단일장애점 (SPOF) 대응 및 Fail-safe 설계 - [진행 예정 / 미완료]
...
* **오프라인 룰베이스(Offline Rule-base) 구동**:
  * 서버 연동 불가 상태가 감지되면 로컬 카메라 노드에 주입되어 있는 기본 규칙에 따라 독자적으로 1차 처리를 수행하도록 대체 제어 로직을 활성화합니다.
...
## 📂 8. 외부 파일 연동을 통한 동적 택배 명단 로드 시스템 도입 - [진행 예정 / 미완료]
...
### 8.2 개선 및 구현 방안
* **동적 임포트 스크립트 (Dynamic Importer)**:
  * 파이썬의 `pandas` 또는 `openpyxl` 라이브러리를 활용하여, 지정한 경로의 엑셀(`.xlsx`) 또는 `.csv` 파일을 업로드하거나 주기적으로 폴링(Polling)하여 입고 정보를 추출하는 모듈을 구축합니다.
* **대시보드 업로드 API 개발**:
  * 웹 대시보드 서버(`dashboard_server.py`)에 파일 업로드용 엔드포인트 `/api/upload_packages`를 구현하여, 현업 관리자가 웹 화면에서 직접 엑셀 파일을 드래그 앤 드롭으로 업로드할 수 있도록 합니다.
* **데이터베이스 자동 동기화(Upsert)**:
  * 업로드된 파일의 필수 열(`package_id`, `customer_name`, `route_zone`)을 파싱하여 PostgreSQL `packages` 테이블에 벌크 인서트(Bulk Insert) 또는 중복 데이터는 업데이트(Upsert)하는 로직을 통합합니다.
  * 파일 반영 완료 시 자동으로 모의 적재 대기 큐(`WAITING` 상태)에 삽입되어, 실시간 ROS2 물류 제어 노드가 재시작 없이 즉각 동작하게 만듭니다.
