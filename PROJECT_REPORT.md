...
1. **입고 및 3방향 초기 분류 (`bg2` 로봇)**:
   * 컨베이어로 들어오는 택배의 ArUco 마커 ID를 스캔하여 **목적지 날짜(YYYY-MM-DD)**를 조회하고, **오늘 날짜(1번 라인), 내일 날짜(2번 라인), 모레 날짜(3번 라인)**에 맞게 분류합니다.
2. **자율 적재 및 파이프라이닝 (`sg2_in_01, 02, 03` 로봇)**:
   * 로봇들이 앞의 2x2 작업대에 택배를 적재하되, 수령인이 이미 창고에 적재되어 있으면 **창고 직송 예외 처리(AMR 단일 상자 이송)**를 수행합니다.
   * 작업대 슬롯이 3개 채워지는 시점에 관제탑에 보고하여 **다음 빈 작업대(ArUco ID 기준)를 미리 로봇 뒤에 대기(Look-ahead)**시킵니다.
...
* **`workstations`**: 2x2 작업대의 현재 물리적 위치, 슬롯별 적재 수령인/상태 및 ArUco 마커 ID(`aruco_id`) 모니터링.
