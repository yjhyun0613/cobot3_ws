...
* **관제 센터 핵심 노드 (`control_tower_node.py`)**:
  ...
  * **데이터베이스 커넥션 풀 도입 (11.1)**: ROS2 병렬 스레드 환경에서 동시 SQL 질의 시 커넥션 획득 대기를 해소하고 자원 누수를 차단하기 위해 `psycopg2.pool.ThreadedConnectionPool`을 활용한 컨텍스트 매니저(`get_db_connection()`) 구조를 도입했습니다.
  ...
  * **교착 방지 규격 고도화**: 무한 블로킹을 차단하기 위해 `wait_for_server` 호출 시 기존 `timeout_sec=1.0`이었던 설정을 **`timeout_sec=5.0`**으로 상향 적용했습니다. 이를 통해 실제 네트워크 분산 연동(실제 AMR 컨트롤 PC와의 통신) 시 발생할 수 있는 ROS 디스커버리 지연으로 인한 오프라인 판정 오류를 완벽하게 예방했습니다.
  ...
  * **물리 레이아웃 및 맵 스케일 일괄 업데이트 (12.1)**:
    * 맵 규격을 중심 `(3.0, 0.0)`, X길이 13.5m, Y길이 20m 범위의 20x20m 규모로 개편했습니다.
