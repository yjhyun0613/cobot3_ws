...
1. **입고 및 3방향 초기 분류 (`bg2` 로봇)**:
   * 컨베이어로 들어오는 택배 바코드를 스캔하여 **목적지 날짜(YYYY-MM-DD)**를 조회하고, **오늘 날짜(1번 라인), 내일 날짜(2번 라인), 모레 날짜(3번 라인)**에 맞게 분류합니다.
2. **자율 적재 및 파이프라이닝 (`sg2_in_01, 02, 03` 로봇)**:
   * 로봇들이 앞의 2x2 작업대에 택배를 적재하되, 수령인이 이미 창고에 적재되어 있으면 **창고 직송 예외 처리(AMR 단일 상자 이송)**를 수행합니다.
   * 작업대 슬롯이 3개 채워지는 시점에 관제탑에 보고하여 **다음 빈 작업대를 미리 로봇 뒤에 대기(Look-ahead)**시킵니다.
...
4. **포장 공정 및 빈 작업대 회수 (`sg2_out_00` 로봇)**:
   * 포장 로봇이 작업을 수행하며, 3번째 슬롯 포장 완료 시점에 관제탑이 이를 감지하여 다음 포장 대기용 작업대를 미리 호출합니다.
   * 포장이 완료되면 고유 출고 ID(`[작업대 ID]+[슬롯]+[날짜]+[시간]`)를 생성해 기록하고 완전히 빈 작업대는 인바운드로 회수합니다.
...
* **`packages`**: 택배 라이프사이클 추적 및 적재된 작업대 번호/슬롯 번호 기록.
* **`workstations`**: 2x2 작업대의 현재 물리적 위치 및 슬롯별 적재 수령인/상태 모니터링.
* **`robots`**: 관제 시스템에 등록된 제어 대상 로봇 목록.

### ② Redis (인메모리 NoSQL - 실시간 제어 및 큐)
* **AMR 상태 해시 (`amr:[id]:status`)**: 실시간 물리적 3D 좌표 및 상태값 캐싱.
* **AMR 명령 큐 (`queue:amr_tasks`)**: 관제탑이 스케줄링하여 AMR에 비동기로 내리는 명령 리스트.
...
* **ROS2 커스텀 인터페이스 (`cobot3_interfaces` 패키지)**:
  * 서비스: `GetPackageRoute.srv`, `CheckWarehouseStatus.srv`, `ReportInboundProgress.srv` (상자 위치 매핑용 `package_id` 포함)
  * 액션: `MovePackage.action`, `ManageWorkstation.action`, `StartPackaging.action`
* **관제 센터 핵심 노드 (`control_tower_node.py`)**:
  * PostgreSQL/Redis 라이브러리 연동 및 멀티스레드 비동기 콜백 적용.
  * 서비스 서버와 액션 클라이언트를 유기적으로 스케줄링하는 `task_scheduler_loop` 탑재.
