...
2. **자율 적재 및 파이프라이닝 (`sg2_in_01, 02, 03` 로봇)**:
   * 로봇들이 앞의 2x4 작업대에 택배를 적재하되, 수령인이 이미 창고에 적재되어 있으면 **창고 직송 예외 처리(AMR 단일 상자 이송)**를 수행합니다.
   * 작업대 슬롯이 7개 채워지는 시점에 관제탑에 보고하여 **다음 빈 작업대(ArUco ID 기준)를 미리 로봇 뒤에 대기(Look-ahead)**시킵니다.
