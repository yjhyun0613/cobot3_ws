...
class ControlTowerNode(Node):
    # AMR 5대 운용 기준: 기존 GUI/Redis/DB 동작 흐름은 유지하고, 이송 명령 폭주만 제한한다.
    ...
    def __init__(self):
        ...
        # Dispatch Gate: 기존 GUI 버튼/Redis 플래그 동작은 유지하고, AMR 작업대 이송 명령 폭주만 제한
        self.scheduler_lock = threading.Lock()
        self.dispatch_lock = threading.Lock()
        self._inside_scheduler_tick = False
        self._scheduler_dispatch_count = 0
    ...
    # ==========================================
    # AMR 작업대 이송 Dispatch Gate
    # ==========================================

    def count_active_workstation_moves(self):
        """현재 AMR이 실제로 이송 중이거나 선점한 작업대 수를 계산한다."""
        if not self.pg_conn_pool:
            return 0
        try:
            with self.get_db_connection() as conn:
            ...
            self.get_logger().error(f'[Dispatch Gate] active move 계산 실패: {str(e)}')
            return self.MAX_ACTIVE_WORKSTATION_MOVES

    def count_available_amrs(self):
        """Redis AMR 상태를 기준으로 사용 가능한 AMR 수를 계산한다. 상태가 없으면 기존 시뮬레이션 기준 5대를 가정한다."""
        if not self.redis_client:
            return self.SIMULATED_AMR_COUNT
        try:
            keys = [k for k in self.redis_client.keys('amr:*') if k != 'queue:amr_tasks']
            if not keys:
                return self.SIMULATED_AMR_COUNT
                ...
                try:
                    ...
                    if key_type == 'hash':
                        ...
                        if val_str:
                            ...
                            state = str(val.get('state', 'IDLE')).upper()
                            available = bool(val.get('available', True))
                            carrying = val.get('carrying_workstation_id', None)
                except Exception:
                    continue

                if available and not carrying and state in ['IDLE', 'WAITING', 'AVAILABLE', 'READY']:
                    available_count += 1

            return max(available_count, 0)
        except Exception as e:
            self.get_logger().error(f'[Dispatch Gate] available AMR 계산 실패: {str(e)}')
            return self.SIMULATED_AMR_COUNT

    def has_workstation_dispatch_capacity(self):
        """작업대 이송 명령을 하나 더 내려도 되는지 판단한다."""
        active_moves = self.count_active_workstation_moves()
        available_amrs = self.count_available_amrs()
        allowed_moves = min(self.MAX_ACTIVE_WORKSTATION_MOVES, max(available_amrs, 1))

        if active_moves >= allowed_moves:
            self.get_logger().info(
                f'[Dispatch Gate] AMR 이송 capacity 초과: active={active_moves}, allowed={allowed_moves}. 신규 이송 보류.'
            )
            return False

        if self._inside_scheduler_tick and self._scheduler_dispatch_count >= self.MAX_WORKSTATION_DISPATCH_PER_TICK:
            self.get_logger().info(
                f'[Dispatch Gate] scheduler tick dispatch 한도 도달: {self._scheduler_dispatch_count}/{self.MAX_WORKSTATION_DISPATCH_PER_TICK}'
            )
            return False

        return True

    def is_target_location_reserved(self, target_location, workstation_id):
        """목적지가 이미 다른 작업대에 의해 점유/이동/회전 중인지 확인한다."""
        if not self.pg_conn_pool:
            return False
        try:
            moving_target = f"MOVING_TO_{target_location.upper()}"
            rotating_target = f"{target_location}_ROTATING"
            with self.get_db_connection() as conn:
                if not conn:
                    return False
                with conn.cursor() as cursor:
                    cursor.execute(
                        ...
                        "  AND current_location IN (%s, %s, %s) "
                        "LIMIT 1;",
                        (workstation_id, target_location, moving_target, rotating_target)
                    )
                    row = cursor.fetchone()
                    return row is not None
        except Exception as e:
            self.get_logger().error(f'[Dispatch Gate] 목적지 중복 검사 실패: {str(e)}')
            return True

    def is_workstation_already_moving(self, workstation_id):
        """같은 작업대가 이미 이동 중이거나 예약되어 있는지 확인한다."""
        if not self.pg_conn_pool:
            return False
        try:
            with self.get_db_connection() as conn:
                ...
                with conn.cursor() as cursor:
                    cursor.execute(
                        "SELECT current_location, reserved_by FROM workstations WHERE workstation_id = %s;",
                        (workstation_id,)
                    )
                    row = cursor.fetchone()
                    if not row:
                        return False
                    current_location, reserved_by = row
                    if current_location and str(current_location).startswith('MOVING_TO_'):
                        return True
                    if reserved_by is not None and str(reserved_by) != '':
                        return True
                    return False
        except Exception as e:
            self.get_logger().error(f'[Dispatch Gate] 작업대 이동 중 검사 실패: {str(e)}')
            return True
    ...
    def task_scheduler_loop(self):
        ...
        if not self.scheduler_lock.acquire(blocking=False):
            self.get_logger().warn('[Scheduler] 이전 scheduler tick이 아직 실행 중이므로 이번 tick은 건너뜁니다.')
            return

        self._inside_scheduler_tick = True
        self._scheduler_dispatch_count = 0
        ...
        try:
            ...
            # 기존 GUI/Redis 버튼 동작은 유지한다. 단, 작업대 이송 capacity가 꽉 찬 경우에는 queue를 pop하지 않고 보류한다.
            try:
                if self.has_workstation_dispatch_capacity():
                    ...
                    self.get_logger().info('[Scheduler] AMR capacity 부족으로 Redis queue pop을 보류합니다.')
    ...
    def trigger_workstation_move(self, workstation_id, start, target, workstation_qr_id="", task_id=None):
        ...
        with self.dispatch_lock:
            if not self.has_workstation_dispatch_capacity():
                self.get_logger().info(
                    f'[Dispatch Gate] capacity 부족으로 작업대 {workstation_id} 이송 보류: {start} -> {target}'
                )
                return False

            if self.is_workstation_already_moving(workstation_id):
                self.get_logger().info(f'[Dispatch Gate] 작업대 {workstation_id}는 이미 이동/예약 중입니다. 이송 생략.')
                return False

            actual_target = target
            actual_start = start
            target_spot_reserved = False
            start_spot_released = False
                            ...
                            with conn.cursor() as cursor:
                                ...
                                if row:
                                    ...
                                    if cursor.rowcount == 0:
                                        self.get_logger().warn(f'[Dispatch Gate] 목적지 스팟 {actual_target} 선점 실패. 이송 보류.')
                                        return False
                                    target_spot_reserved = True
                                    self.get_logger().info(f'[DB] 작업대 {workstation_id}의 스팟 배정: {actual_target}')
                                else:
                                    self.get_logger().error(f"경고: {target} 구역에 빈 주차 공간이 없습니다. 이송을 보류합니다.")
                                    return False
                        except Exception as e:
                            self.get_logger().error(f'{target} 스팟 조회 및 업데이트 중 에러: {str(e)}')
                            return False

            if self.is_target_location_reserved(actual_target, workstation_id):
                self.get_logger().info(f'[Dispatch Gate] 목적지 {actual_target}는 이미 다른 작업대가 점유/이동/회전 중입니다. 이송 보류.')
                    ...
                    if conn:
                        try:
                            with conn.cursor() as cursor:
                                ...
                                start_spot_released = cursor.rowcount > 0
            ...
            with self.get_db_connection() as conn:
                if conn:
                    try:
                        with conn.cursor() as cursor:
                            cursor.execute(
                                "UPDATE workstations SET current_location = %s, status = 'PROCESSING', reserved_by = 'AMR_01' "
                                "WHERE workstation_id = %s "
                            ...
                            if cursor.rowcount == 0:
                                self.get_logger().warn(
                                    f'[Dispatch Gate] 작업대 {workstation_id} 출발 위치가 변경되었거나 이미 예약됨. 이송 보류.'
                                )
                                self.recover_workstation_move_db_state(workstation_id, actual_start, actual_target)
                                return False
                    except Exception as e:
        ...
        if not target_coords_found:
            self.get_logger().error(f'[Dispatch Gate] 목적지 {actual_target}의 floor_qr_map 좌표가 없습니다. 이송을 취소하고 DB를 복구합니다.')
            self.recover_workstation_move_db_state(workstation_id, actual_start, actual_target)
            return False
        ...
        try:
            ...
            if self._inside_scheduler_tick:
                self._scheduler_dispatch_count += 1
            return True
        except Exception as e:
            self.get_logger().error(f'작업대 {workstation_id} 이송 Action 전송 실패: {str(e)}')
            self.recover_workstation_move_db_state(workstation_id, actual_start, actual_target)
            return False
