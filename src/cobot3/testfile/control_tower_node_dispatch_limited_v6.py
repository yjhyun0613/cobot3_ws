...
class ControlTowerNode(Node):
    ...
    MAX_WORKSTATION_DISPATCH_PER_TICK = 5
    MAX_REDIS_TASKS_PER_TICK = 5
    SIMULATED_AMR_COUNT = 5
    ASSUME_MISSING_SIM_AMRS_AVAILABLE = True
    AMR_REDIS_KEY_PREFIX = 'amr:'
    ...
    def __init__(self):
        ...
        # AMR 5대 동시 운용 기준 dispatch backpressure 상태
        self.scheduler_lock = Lock()
        self.dispatch_lock = Lock()
        self.dispatched_this_tick = 0
        self._last_workstation_move_block_reason = ""
        self.inbound_pre_fetch_triggered = set()
        self.inbound_rotation_triggered = set()
    ...
    # ==========================================
    # AMR Dispatch Backpressure / Throttle
    # ==========================================

    def _is_moving_location(self, location):
        return isinstance(location, str) and location.startswith('MOVING_TO_')

    def _is_rotating_location_for(self, location, base_location):
        return isinstance(location, str) and location == f'{base_location}_ROTATING'

    def _safe_redis_key_type(self, key):
        try:
    ...
    def _safe_read_redis_mapping(self, key):
        """Redis amr:* 상태를 Hash/String JSON 둘 다 안전하게 읽는다.

        기존 코드처럼 hgetall을 먼저 호출하면 해당 key가 String일 때 WRONGTYPE 로그가 반복될 수 있다.
        Isaac AMR 상태가 hash로 올라오든 JSON string으로 올라오든 같은 dict 형태로 정규화한다.
        """
        if not self.redis_client:
            return {}
        try:
            key_type = self._safe_redis_key_type(key)
            if key_type == 'hash':
            ...
            self.get_logger().error(f'[Redis] AMR state read 실패: key={key}, err={str(e)}')
            return {}

    def _iter_amr_redis_keys(self):
        if not self.redis_client:
            return []
        try:
            return list(self.redis_client.scan_iter(match=f'{self.AMR_REDIS_KEY_PREFIX}*', count=50))
        except Exception:
            try:
                return self.redis_client.keys(f'{self.AMR_REDIS_KEY_PREFIX}*')
            except Exception as e:
                self.get_logger().error(f'[Redis] AMR key scan 실패: {str(e)}')
                return []

    def count_active_workstation_moves(self):
        """현재 AMR이 수행 중인 작업대 이송 수를 계산한다.

        status='PROCESSING'만으로는 active move로 보지 않는다.
        sg2_out_00_A에 도착한 작업대는 포장 중에도 PROCESSING일 수 있기 때문이다.
        실제 이송 중 판단 기준은 MOVING_TO_* 또는 reserved_by 존재 여부다.
        """
        if not self.pg_conn_pool:
            return 0
            ...
            try:
                with conn.cursor() as cursor:
                    cursor.execute(
                        ...
                        "   OR (reserved_by IS NOT NULL AND reserved_by <> '');"
                    )
                    return int(cursor.fetchone()[0])
            except Exception as e:
                self.get_logger().error(f'[Dispatch Gate] active move count 조회 실패: {str(e)}')
                return self.MAX_ACTIVE_WORKSTATION_MOVES

    def count_available_amrs(self):
        """Redis의 amr:* 상태를 기준으로 사용 가능한 AMR 수를 계산한다.

        Hash/String JSON 둘 다 지원한다. Redis 상태가 아직 없으면 시뮬레이션 초기 상태로 보고
        최대 5대를 사용 가능하다고 본다.
        """
        if not self.redis_client:
            return self.MAX_ACTIVE_WORKSTATION_MOVES
        ...
        try:
            keys = self._iter_amr_redis_keys()
            available_count = 0
            observed_count = 0

            for key in keys:
                key_str = key.decode('utf-8') if isinstance(key, bytes) else str(key)
                val = self._safe_read_redis_mapping(key_str)
                if not val:
                    continue

                observed_count += 1
                state = str(val.get('state', 'IDLE')).upper()
                available_raw = str(val.get('available', 'true')).lower()
                carrying = val.get('carrying_workstation_id', None)
                available = available_raw in ['true', '1', 'yes']

                if available and state in ['IDLE', 'WAITING', 'AVAILABLE'] and not carrying:
                    available_count += 1

            if observed_count == 0:
                return self.MAX_ACTIVE_WORKSTATION_MOVES

            if self.ASSUME_MISSING_SIM_AMRS_AVAILABLE and observed_count < self.SIMULATED_AMR_COUNT:
                available_count += self.SIMULATED_AMR_COUNT - observed_count

            return max(0, min(self.MAX_ACTIVE_WORKSTATION_MOVES, available_count))
        except Exception as e:
            self.get_logger().error(f'[Dispatch Gate] Redis AMR available count 조회 실패: {str(e)}')
            return self.MAX_ACTIVE_WORKSTATION_MOVES

    def get_workstation_dispatch_budget(self):
        active_moves = self.count_active_workstation_moves()
        available_amrs = self.count_available_amrs()

        # active_moves는 이미 DB에 반영된 MOVING/reserved 작업 수다.
        # dispatched_this_tick까지 더하면 같은 tick에서 방금 보낸 작업을 이중 계산하게 된다.
        remaining_by_active = self.MAX_ACTIVE_WORKSTATION_MOVES - active_moves

        # Redis의 AMR 상태는 Action 전송 직후 바로 BUSY로 바뀌지 않을 수 있으므로,
        # 이번 tick에서 이미 보낸 수만큼은 available_amrs에서 차감한다.
        remaining_by_amr = available_amrs - self.dispatched_this_tick
        remaining_by_tick = self.MAX_WORKSTATION_DISPATCH_PER_TICK - self.dispatched_this_tick
        return max(0, min(remaining_by_active, remaining_by_amr, remaining_by_tick))

    def _set_dispatch_block_reason(self, reason):
        self._last_workstation_move_block_reason = reason

    def can_dispatch_new_workstation_move(self, workstation_id, target_location=''):
        """새 작업대 이송 명령을 지금 보낼 수 있는지 검사한다.

        목표: AMR 5대 병렬성은 유지하되 bridge_queue/commands 폭주를 방지한다.
        """
        self._set_dispatch_block_reason('')

        with self.dispatch_lock:
            active_moves = self.count_active_workstation_moves()
            available_amrs = self.count_available_amrs()

            if active_moves >= self.MAX_ACTIVE_WORKSTATION_MOVES:
                self._set_dispatch_block_reason('capacity')
                self.get_logger().warn(
                    f'[Dispatch Gate] active move limit 도달: '
                    f'active={active_moves}, max={self.MAX_ACTIVE_WORKSTATION_MOVES}. '
                    f'{workstation_id} -> {target_location} 보류.'
                )
                return False

            if self.dispatched_this_tick >= self.MAX_WORKSTATION_DISPATCH_PER_TICK:
                self._set_dispatch_block_reason('capacity')
                self.get_logger().warn(
                    f'[Dispatch Gate] scheduler tick dispatch limit 도달: '
                    f'this_tick={self.dispatched_this_tick}, max_tick={self.MAX_WORKSTATION_DISPATCH_PER_TICK}. '
                    f'{workstation_id} -> {target_location} 보류.'
                )
                return False

            if available_amrs - self.dispatched_this_tick <= 0:
                self._set_dispatch_block_reason('capacity')
                self.get_logger().warn(
                    f'[Dispatch Gate] 사용 가능한 AMR 없음 또는 이번 tick AMR budget 소진. '
                    f'available={available_amrs}, this_tick={self.dispatched_this_tick}. '
                    f'{workstation_id} -> {target_location} 보류.'
                )
                return False
        ...
        with self.get_db_connection() as conn:
            ...
            try:
                with conn.cursor() as cursor:
                    cursor.execute(
                        "SELECT current_location, status, reserved_by FROM workstations WHERE workstation_id = %s;",
                        (workstation_id,)
                    )
                    row = cursor.fetchone()
                    if not row:
                        self._set_dispatch_block_reason('invalid_workstation')
                        self.get_logger().warn(f'[Dispatch Gate] 작업대 {workstation_id}를 DB에서 찾을 수 없어 이송 보류.')
                        return False

                    current_location, status, reserved_by = row
                    if self._is_moving_location(current_location) or (reserved_by is not None and str(reserved_by) != ''):
                        self._set_dispatch_block_reason('already_active')
                        self.get_logger().info(
                            f'[Dispatch Gate] 작업대 {workstation_id}는 이미 이송/예약 중입니다. '
                            f'loc={current_location}, status={status}, reserved_by={reserved_by}'
                        )
                        return False

                    if target_location and target_location not in ['warehouse', 'staging']:
                        cursor.execute(
                            "SELECT COUNT(*) FROM workstations "
                            "WHERE workstation_id <> %s "
                            "  AND (current_location = %s OR current_location = %s OR current_location = %s);",
                            (
                                workstation_id,
                                target_location,
                                f'MOVING_TO_{target_location.upper()}',
                                f'{target_location}_ROTATING'
                            )
                        )
                        if int(cursor.fetchone()[0]) > 0:
                            self._set_dispatch_block_reason('target_reserved')
                            self.get_logger().info(
                                f'[Dispatch Gate] 목적지 {target_location}가 이미 점유/예약되어 {workstation_id} 이송 보류.'
                            )
                            return False

                    return True
            except Exception as e:
                self._set_dispatch_block_reason('db_error')
                self.get_logger().error(f'[Dispatch Gate] dispatch 가능 여부 검사 실패: {str(e)}')
                return False

    def register_workstation_dispatch(self):
        with self.dispatch_lock:
            self.dispatched_this_tick += 1

    def requeue_amr_task(self, task, reason='capacity'):
        """capacity 부족으로 실행하지 못한 Redis task를 같은 uuid로 재삽입한다."""
        if not self.redis_client:
            return
        try:
            task_type = task.get('task_type', 'TASK')
            score = self.get_task_priority(task_type)
            task['defer_count'] = int(task.get('defer_count', 0)) + 1
            self.redis_client.zadd('queue:amr_tasks', {json.dumps(task): score})
            self.get_logger().info(
                f'[Redis Queue] capacity 부족으로 task 재대기: {task_type}, '
                f'uuid={task.get("uuid", "")}, defer_count={task.get("defer_count")}, reason={reason}'
            )
        except Exception as e:
            self.get_logger().error(f'[Redis Queue] task 재대기 실패: {str(e)}')
    ...
    def report_inbound_progress_callback(self, request, response):
        ...
        # 같은 progress report가 여러 번 들어와도 Redis queue에 중복 PRE_FETCH가 쌓이지 않도록 방지한다.
        if filled_slots_count == 3:
            prefetch_key = (workstation_id, robot_id, 3)
            if prefetch_key in self.inbound_pre_fetch_triggered:
                self.get_logger().info(f'[Look-ahead] {workstation_id} 3번째 슬롯 사전 호출은 이미 처리됨. 중복 생략.')
            else:
                self.inbound_pre_fetch_triggered.add(prefetch_key)
        ...
        # 동일 작업대/로봇의 4번째 슬롯 report 반복 수신 시 ROTATE task 중복 생성 방지.
        if filled_slots_count == 4:
            rotation_key = (workstation_id, robot_id, 4)
            if rotation_key in self.inbound_rotation_triggered:
                self.get_logger().info(f'[Rotation Trigger] {workstation_id} 4번째 슬롯 회전은 이미 처리됨. 중복 생략.')
                response.success = True
                return response
            self.inbound_rotation_triggered.add(rotation_key)
    ...
    def get_task_priority(self, task_type):
        ...
        if task_type in ['DIRECT_WAREHOUSE', 'RETRIEVE_FULL_WORKSTATION', 'ROTATE_WORKSTATION']:
            ...
            return 80
        elif task_type in ['PRE_FETCH_PACKAGING_WORKSTATION', 'PRE_FETCH_WORKSTATION']:
            return 70
    ...
    def _task_signature(self, task_dict):
        """Redis queue 중복 삽입 방지용 안정적인 task signature."""
        task_type = task_dict.get('task_type', 'TASK')
        if task_type == 'DIRECT_WAREHOUSE':
            return (task_type, task_dict.get('package_id', ''), task_dict.get('destination_zone', ''))
        if task_type == 'PRE_FETCH_EMPTY_WORKSTATION':
            return (task_type, task_dict.get('target_robot', ''))
        if task_type in ['PRE_FETCH_WORKSTATION', 'ROTATE_WORKSTATION', 'RETRIEVE_EMPTY_WORKSTATION']:
            return (
                task_type,
                task_dict.get('workstation_id', ''),
                task_dict.get('from', ''),
                task_dict.get('to', '')
            )
        return (task_type, task_dict.get('workstation_id', ''), task_dict.get('package_id', ''))

    def _redis_queue_has_task_signature(self, task_dict):
        """queue:amr_tasks 안에 같은 의미의 작업이 이미 대기 중인지 확인."""
        if not self.redis_client:
            return False
        target_sig = self._task_signature(task_dict)
        try:
            raw_items = self.redis_client.zrange('queue:amr_tasks', 0, -1)
            for raw in raw_items:
                try:
                    queued = json.loads(raw)
                except Exception:
                    continue
                if self._task_signature(queued) == target_sig:
                    return True
        except Exception as e:
            if "WRONGTYPE" in str(e):
                self.redis_client.delete('queue:amr_tasks')
            else:
                self.get_logger().error(f'[Redis Queue] 중복 검사 실패: {str(e)}')
        return False

    def push_amr_task(self, task_dict):
        """AMR 작업 명령을 Redis 대기 큐(Sorted Set)에 집어넣음.

        동일 의미의 작업이 이미 queue에 있으면 중복 삽입하지 않는다.
        반복 service feedback 때문에 PRE_FETCH/ROTATE task가 누적되는 문제를 방지한다.
        """
        import uuid
        task_type = task_dict.get('task_type', 'TASK')
        score = self.get_task_priority(task_type)

        # 기존 uuid가 있으면 보존한다. requeue/외부 추적 task_id가 바뀌지 않게 하기 위함.
        task_uuid = task_dict.get('uuid') or str(uuid.uuid4())
        task_dict['uuid'] = task_uuid

        if self.redis_client:
            try:
                if self._redis_queue_has_task_signature(task_dict):
                    self.get_logger().info(f'[Redis Queue] 중복 AMR 태스크 감지, 추가 생략 -> {task_type}, sig={self._task_signature(task_dict)}')
                    return task_uuid
    ...
    def task_scheduler_loop(self):
        ...
        if not self.scheduler_lock.acquire(blocking=False):
            self.get_logger().warn('[Scheduler] 이전 scheduler tick이 아직 실행 중이므로 이번 tick을 건너뜁니다.')
            return
        ...
        try:
            ...
            #    capacity가 남아 있으면 한 tick에서 최대 5개까지 꺼내 5대 병렬성을 살린다.
            #    capacity가 꽉 찬 경우에는 pop하지 않아 task 유실/반복 재삽입을 막는다.
            try:
                popped_count = 0
                while popped_count < self.MAX_REDIS_TASKS_PER_TICK and self.get_workstation_dispatch_budget() > 0:
                    ...
                    popped_count += 1
                    self.get_logger().info(
                        f'[Scheduler] Redis 큐에서 최고 우선순위 태스크 감지 '
                        f'(Score: {score}, pop={popped_count}/{self.MAX_REDIS_TASKS_PER_TICK}) -> {task.get("task_type", "TASK")}'
                    )
                    self.execute_amr_task(task)

                if popped_count == 0:
                    queued_count = self.redis_client.zcard('queue:amr_tasks')
                    if queued_count and self.get_workstation_dispatch_budget() <= 0:
                        self.get_logger().info(f'[Scheduler] AMR capacity full. Redis 대기 task 유지 중: {queued_count}개')
    ...
    def mark_workstation_packages_in_workstation(self, workstation_id):
        """창고/staging에 있던 완충 작업대가 포장존으로 실제 dispatch된 뒤 패키지 상태를 복원한다.

        dispatch 전에는 호출하지 않는다. capacity 부족으로 보류된 작업대의 패키지가
        IN_WORKSTATION으로 미리 바뀌는 문제를 막기 위한 후처리다.
        """
        if not self.pg_conn_pool:
            return
        with self.get_db_connection() as conn:
            ...
            try:
                with conn.cursor() as cursor:
                    cursor.execute(
                        "UPDATE packages SET status = 'IN_WORKSTATION' "
                        "WHERE workstation_id = %s AND status = 'IN_WAREHOUSE';",
                        (workstation_id,)
                    )
                    if cursor.rowcount > 0:
                        self.get_logger().info(
                            f'[DB] 작업대 {workstation_id} 패키지 상태를 IN_WORKSTATION으로 복원 완료. count={cursor.rowcount}'
                        )
            except Exception as e:
                self.get_logger().error(f'[DB] 패키지 상태 복원 실패: {str(e)}')
    ...
    def execute_amr_task(self, task):
        ...
        if task_type == 'DIRECT_WAREHOUSE':
            ...
            if workstation_id != 'WS_TEMP_EMPTY':
                dispatched = self.trigger_workstation_move(workstation_id, start_location, target_location, workstation_qr_id, task_id=task_id)
                if not dispatched and self._last_workstation_move_block_reason == 'capacity':
                    self.requeue_amr_task(task, reason='capacity')
            else:
            ...
            dispatched = self.trigger_workstation_move(workstation_id, task['from'], task['to'], task.get('workstation_qr_id', ""), task_id=task_id)
            if dispatched and str(task.get('to', '')).startswith('sg2_out_'):
                self.mark_workstation_packages_in_workstation(workstation_id)
            elif not dispatched and self._last_workstation_move_block_reason == 'capacity':
                self.requeue_amr_task(task, reason='capacity')
            ...
            # 포장 완료 후 빈 작업대를 창고/대기 위치로 회수하는 액션
            workstation_id = task['workstation_id']
            dispatched = self.trigger_workstation_move(workstation_id, task.get('from', 'sg2_out_00_A'), task.get('to', 'warehouse'), task.get('workstation_qr_id', ""), task_id=task_id)
            if not dispatched and self._last_workstation_move_block_reason == 'capacity':
                self.requeue_amr_task(task, reason='capacity')
                    ...
                    with conn.cursor() as cursor:
                        ...
                        for row in rows:
                            ...
                            if curr_loc == 'sg2_in_01_A':
                                cursor.execute(
                                    "SELECT COUNT(*) FROM workstations "
                                    "WHERE current_location = 'sg2_out_00_A' OR current_location = 'MOVING_TO_SG2_OUT_00_A' OR current_location = 'sg2_out_00_A_ROTATING';"
                                )
                                out_a_count = cursor.fetchone()[0]
                                
                                cursor.execute(
                                    "SELECT COUNT(*) FROM workstations "
                                    "WHERE current_location = 'sg2_out_00_B' OR current_location = 'MOVING_TO_SG2_OUT_00_B' OR current_location = 'sg2_out_00_B_ROTATING';"
                                )
                    ...
                    with conn.cursor() as cursor:
                        ...
                        for ws_id, loc, qr_id in workstations:
                            ...
                            for line in inbound_lines:
                                ...
                                elif loc == f"{line}_A_ROTATING":
                                    line_status[line]['A'].append((ws_id, ws_qr))
                                elif loc == f"{line}_B_ROTATING":
                                    line_status[line]['B'].append((ws_id, ws_qr))
                            ...
                            elif loc == 'sg2_out_00_A_ROTATING':
                                sg2_out_status['A'].append((ws_id, ws_qr))
                            elif loc == 'sg2_out_00_B_ROTATING':
                                sg2_out_status['B'].append((ws_id, ws_qr))
                        ...
                        if not sg2_out_status['A'] and not sg2_out_status['MOVING_A']:
                            ...
                            # B구역에도 없는 경우 -> 창고에서 완충된 작업대를 조회해서 A구역으로 바로 공급 (오늘 물량만)
                                ...
                                if row:
                                    ...
                                    dispatched = self.trigger_workstation_move(ws_id, start_loc, 'sg2_out_00_A', ws_qr)
                                    if dispatched:
                                        self.mark_workstation_packages_in_workstation(ws_id)
        except Exception as e:
            self.get_logger().error(f'Keep-Alive Dispatcher 실행 중 에러: {str(e)}')

    def rollback_pre_dispatch_reservations(self, workstation_id, start, target):
        """Action Goal 전송 전 실패 시 warehouse/staging spot 선점/해제를 되돌린다.

        이 함수는 workstations.current_location을 강제로 건드리지 않는다.
        아직 workstation 이동 상태 UPDATE 전 단계에서 실패했을 때 source/target parking slot만 복구하기 위한 방어 로직이다.
        """
        if not self.pg_conn_pool:
            return
        with self.get_db_connection() as conn:
            ...
            try:
                with conn.cursor() as cursor:
                    if isinstance(start, str) and (start.startswith('spot_') or start.startswith('stage_')):
                        cursor.execute(
                            "UPDATE warehouse_locations SET status = 'OCCUPIED', workstation_id = %s "
                            "WHERE spot_id = %s AND (workstation_id IS NULL OR workstation_id = %s OR status = 'EMPTY');",
                            (workstation_id, start, workstation_id)
                        )
                        if cursor.rowcount > 0:
                            self.get_logger().info(f'[DB 복구] 출발 스팟 {start} 재점유 복구 완료.')
                        else:
                            self.get_logger().warn(f'[DB 복구] 출발 스팟 {start}가 다른 작업대에 의해 점유되어 복구를 건너뜁니다.')
                    if isinstance(target, str) and (target.startswith('spot_') or target.startswith('stage_')):
                        cursor.execute(
                            "UPDATE warehouse_locations SET status = 'EMPTY', workstation_id = NULL WHERE spot_id = %s AND workstation_id = %s;",
                            (target, workstation_id)
                        )
                        self.get_logger().info(f'[DB 복구] 목적 스팟 {target} 선점 해제 복구 완료.')
            except Exception as e:
                self.get_logger().error(f'[DB 복구] pre-dispatch reservation rollback 실패: {str(e)}')
    ...
    def trigger_workstation_move(self, workstation_id, start, target, workstation_qr_id="", task_id=None):
        ...
        AMR 5대 운용 기준으로 active workstation move가 5개를 넘지 않도록
        dispatch gate를 통과한 경우에만 Action Goal을 전송한다.
        """
        import uuid
        if not task_id:
        ...
        # 0차 dispatch gate: 현재 active move / AMR availability / 동일 WS 예약 여부 검사
        if not self.can_dispatch_new_workstation_move(workstation_id, target):
            return False
                    ...
                    try:
                        with conn.cursor() as cursor:
                            ...
                            if row:
                                ...
                                # 다른 tick/노드가 같은 스팟을 먼저 잡은 경우를 방지하기 위해 EMPTY 조건을 다시 확인한다.
                                cursor.execute(
                                    "UPDATE warehouse_locations SET status = 'OCCUPIED', workstation_id = %s "
                                    "WHERE spot_id = %s AND status = 'EMPTY';",
                                    (workstation_id, actual_target)
                                )
                                if cursor.rowcount == 0:
                                    self.get_logger().warn(
                                        f'[DB] 목적 스팟 {actual_target}가 이미 선점되어 {workstation_id} 이송을 보류합니다.'
                                    )
                                    self._set_dispatch_block_reason('target_full')
                                    return False
                                self.get_logger().info(f'[DB] 작업대 {workstation_id}의 스팟 배정: {actual_target}')
                            else:
                                self.get_logger().error(f"경고: {target} 구역에 빈 주차 공간이 없습니다. {workstation_id} 이송을 보류합니다.")
                                self._set_dispatch_block_reason('target_full')
                                return False
                    except Exception as e:
                        self.get_logger().error(f'{target} 스팟 조회 및 업데이트 중 에러: {str(e)}')
                        self._set_dispatch_block_reason('db_error')
                        self.rollback_pre_dispatch_reservations(workstation_id, actual_start, actual_target)
                        return False

        # 실제 target이 정해진 후 2차 target 예약 검사
        if not self.can_dispatch_new_workstation_move(workstation_id, actual_target):
            # warehouse/staging 스팟을 이미 선점했다면 복구
            if actual_target.startswith('spot_') or actual_target.startswith('stage_'):
                with self.get_db_connection() as conn:
                    if conn:
                        try:
                            with conn.cursor() as cursor:
                                cursor.execute(
                                    ...
                                    "WHERE spot_id = %s AND workstation_id = %s;",
                                    (actual_target, workstation_id)
                                )
                        except Exception as e:
                            self.get_logger().error(f'[Dispatch Gate] target 선점 복구 실패: {str(e)}')
            return False
                ...
                if conn:
                    try:
                        with conn.cursor() as cursor:
                            cursor.execute(
                                "UPDATE warehouse_locations SET status = 'EMPTY', workstation_id = NULL "
                                "WHERE spot_id = %s AND (workstation_id = %s OR workstation_id IS NULL);",
                                (actual_start, workstation_id)
                            )
                            if cursor.rowcount == 0:
                                cursor.execute(
                                ...
                                row = cursor.fetchone()
                                self.get_logger().warn(
                                    f'[DB] 출발 스팟 {actual_start}가 {workstation_id} 소유가 아니어서 이송을 보류합니다. '
                                    f'current={row}'
                                )
                                self._set_dispatch_block_reason('stale_start_or_already_active')
                                self.rollback_pre_dispatch_reservations(workstation_id, actual_start, actual_target)
                                return False
            ...
            if conn:
                try:
                    with conn.cursor() as cursor:
                        cursor.execute(
                            "UPDATE workstations "
                            "SET current_location = %s, status = 'PROCESSING', reserved_by = 'AMR_POOL' "
                            "WHERE workstation_id = %s "
                            "  AND current_location = %s "
                            "  AND (current_location IS NULL OR current_location NOT LIKE 'MOVING_TO_%%') "
                            "  AND (reserved_by IS NULL OR reserved_by = '');",
                            (f"MOVING_TO_{actual_target.upper()}", workstation_id, actual_start)
                        )
                        if cursor.rowcount == 0:
                            self.get_logger().info(
                                f'[Dispatch Gate] 작업대 {workstation_id}의 출발지/예약 상태가 변경되어 이송을 생략합니다. '
                                f'expected_start={actual_start}'
                            )
                            self._set_dispatch_block_reason('stale_start_or_already_active')
                            self.rollback_pre_dispatch_reservations(workstation_id, actual_start, actual_target)
                            return False
        ...
        # 목적지 좌표가 없으면 Isaac 쪽에서 (0,0)으로 잘못 이동할 수 있으므로 전송하지 않는다.
        if not target_coords_found:
            self.get_logger().error(
                f'[Dispatch Gate] floor_qr_map에 목적지 {actual_target} 좌표가 없어 {workstation_id} 이송을 취소합니다.'
            )
            self._set_dispatch_block_reason('target_coord_missing')
        ...
        if not self.manage_workstation_action_client.wait_for_server(timeout_sec=self.ACTION_SERVER_WAIT_TIMEOUT_SEC):
            ...
            self.publish_task_event(
                ...
                status='FAILED',
                assigned_amr='AMR_POOL'
            )
            # Reset workstation status in PG
            self.recover_workstation_move_db_state(workstation_id, actual_start, actual_target)
            self._set_dispatch_block_reason('server_unavailable')
            return False
        ...
        try:
            ...
            self.get_logger().error(f'AMR Action Goal 전송 실패: {str(e)}')
            self.recover_workstation_move_db_state(workstation_id, actual_start, actual_target)
            self._set_dispatch_block_reason('action_send_error')
            return False

        self.register_workstation_dispatch()
        self._set_dispatch_block_reason('')
        return True
                ...
                try:
                    with conn.cursor() as cursor:
                        ...
                        if start.startswith('spot_') or start.startswith('stage_'):
                            ...
                            if cursor.rowcount == 0:
                                self.get_logger().warn(f'[DB 복구] 출발지 스팟 {start}가 다른 작업대에 의해 점유되어 재점유 복구를 건너뜁니다.')
                            cursor.execute(
                                "UPDATE packages SET status = 'IN_WAREHOUSE' "
                                "WHERE workstation_id = %s AND status = 'IN_WORKSTATION';",
                                (workstation_id,)
                            )
                            self.get_logger().info(f'[DB 복구] 출발지 스팟 {start} 재점유 및 패키지 창고 상태 복구 완료.')
                        ...
                        if target.startswith('spot_') or target.startswith('stage_'):
                            ...
                            if cursor.rowcount > 0:
                                ...
                                self.get_logger().warn(f'[DB 복구] 목적지 스팟 {target}가 이미 다른 상태이므로 선점 해제를 건너뜁니다.')
                except Exception as e:
    ...
    def workstation_move_response_callback(self, future, workstation_id, start, target, task_id):
        ...
        try:
            ...
            self.get_logger().error(f'작업대 {workstation_id} 이송 goal response 처리 실패: {str(e)}')
    ...
    def workstation_move_completed_callback(self, future, workstation_id, start, target, task_id):
        ...
        try:
            ...
            self.get_logger().error(f'작업대 {workstation_id} 이송 result 처리 실패: {str(e)}')
            self.recover_workstation_move_db_state(workstation_id, start, target)
            return
                    ...
                    try:
                        with conn.cursor() as cursor:
                            ...
                            if row:
                                ...
                                # 패키지 상태는 여기서 미리 바꾸지 않는다.
                                # capacity 부족으로 task가 pending되면 창고에 있는 작업대가 IN_WORKSTATION으로 보이는 문제가 생긴다.
                                # 실제 /manage_workstation dispatch 성공 후 mark_workstation_packages_in_workstation()에서 변경한다.
                                self.push_amr_task({
    ...
    def packaging_completed_callback(self, future, workstation_id):
        ...
        if result.success:
            ...
            # 빈 작업대 회수도 직접 dispatch하지 않고 Redis queue에 넣는다.
            # Action callback에서 바로 trigger_workstation_move()를 호출하면 scheduler tick budget과 경합할 수 있다.
            self.push_amr_task({
                ...
                'description': f'포장 완료 후 빈 작업대 {workstation_id} 창고 회수 대기'
            })
    ...
    def publish_fleet_states_callback(self):
        ...
        if self.redis_client:
            try:
                for key in self._iter_amr_redis_keys():
                    key_str = key.decode('utf-8') if isinstance(key, bytes) else str(key)
                    parts = key_str.split(":")
                    if len(parts) > 1:
                        amr_id = parts[1]
                        ...
                        if val:
                            amr_states[amr_id] = {
                                ...
                                "available": str(val.get("available", "true")).lower() in ["true", "1", "yes"]
                            }
            except Exception as e:
        ...
        if not amr_states:
            ...
            amr_states = {
                ...
                for idx in range(1, self.SIMULATED_AMR_COUNT + 1)
            }
