...
import json
import time
import threading  # 다중 스레드 동기화를 위한 threading 모듈 임포트
...
class ControlTowerNode(Node):
    def __init__(self):
        ...
        # 1. 데이터베이스 연결 정보 및 스레드 락 설정
        self.pg_conn = None
        self.redis_client = None
        # 중첩 호출 시 데드락을 방지하기 위해 재진입 가능 락(RLock) 사용
                ...
                with self.pg_conn.cursor() as cursor:
                    ...
                    for row in rows:
                        ...
                        if curr_loc == 'sg2_in_01_A':
                            # 오늘 물량: 포장존 A 직행 가능하면 직행, 아니면 출고 대기 창고(staging)에 대기
                            cursor.execute(
                                "SELECT COUNT(*) FROM workstations "
                            ...
                            if out_a_count == 0:
                                ...
                                target_out = 'staging'  # 포장존이 사용 중이면 출고 대기 창고에 대기
                            ...
                            self.get_logger().info(f'[Scheduler] {ws_id}(QR: {ws_qr_id}) 내일 물량 적재 완료! 출고 대기 창고(staging) 보관 스케줄링 시작.')
    ...
    def trigger_packaging_process(self, workstation_id):
        """포장 로봇 sg2_out_00에게 포장 개시 명령 손신"""
        workstation_qr_id = ""
        if self.pg_conn:
            try:
                with self.pg_lock:
    ...
    def packaging_feedback_callback(self, feedback_msg):
        """포장 로봇의 실시간 피드백 핸들러 (7번째 칸 Look-ahead 최적화 사양 동기화 완료)"""
        ...
        # [Look-ahead 최적화] 7번째 칸 포장 완료 시 다음 작업대 B구역으로 사전 호출 (문서 스펙 사양 일치화)
        if completed_slots == 7 and workstation_id not in self.pre_fetch_triggered:
            self.pre_fetch_triggered.add(workstation_id)
            self.get_logger().info(f'[Look-ahead] 7번째 칸 포장 완료 감지! 작업대 {workstation_id}에 대해 다음 작업대 B구역 사전 호출을 시작합니다.')
    ...
    def write_daily_report(self, date_str, cursor):
        ...
        try:
            ...
            report_content.append("\n## 출고 내역 리스트")
            report_content.append("| 패키지 ID | 수령인 | 출고 ID |")
            report_content.append("| :--- | :--- | :--- |")
